...
#include <utility>
namespace fem {
struct AssembledSystem { Eigen::MatrixXd stiffness; Eigen::VectorXd load; DofManager dofs; };
class Model {
 public:
  ...
  ElementType& addElement(Args&&... args) {
    ...
    if (elements_.count(id)!=0U) throw std::invalid_argument("Duplicate element id");
